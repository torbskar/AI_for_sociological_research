...
**Target**: *Sociological Methods & Research* (SMR, SAGE) — methodological contribution
...
**Function**: Brief literature grounding — not a survey. Maps what exists, names the gap, and positions the paper's contribution. Keep short; SMR has a 10,000-word limit including references — every word in the literature review is expensive.
...
- **The conservative framing**: Extending existing replication package norms to AI-assisted workflows requires no new principle and no new infrastructure — only an expanded understanding of what "the materials needed to reproduce the analysis" includes. For a study using regression, that means code and data. For a study using a structured AI workflow, that means skill files, prompt templates, search scripts, and screening logs. Journals that already require replication packages (SMR's Open Science Policy, June 2025) have accepted the underlying logic; they have no principled basis for treating AI workflow documentation differently. This makes the policy recommendation conservative rather than radical.
- **Note on SMR's own AI policy**: SAGE's disclosure-based AI policy (distinguishes assistive from generative AI) is more sophisticated than a blanket ban, and the concessive move must acknowledge this before pressing the critique. The argument: even SAGE's more nuanced approach leaves the structured/unstructured distinction unaddressed. A researcher who uses generative AI in a wholly undocumented, unverifiable way and one who uses it systematically with documented criteria and human verification checkpoints face the same disclosure requirement. That is the gap the paper fills.
