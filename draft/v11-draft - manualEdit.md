...
Engzell and Wilmers (2026) describe a limiting case that confirms and complicates this picture. Their fully automated multi-agent pipeline — the 'Paper Factory' — can only run if the researcher has first codified all heuristics into explicit, executable scaffolding. Full automation enforces externalisation as an architectural requirement, and the design confirms the argument. But it also clarifies what externalisation cannot reach. Expert scaffolding captures implicit tacit knowledge — criteria, frameworks, methodological preferences — precisely because this is what the researcher can articulate if pressed. The inherent component — pattern-recognition and contextual judgment that resist full articulation in principle — operates differently. In a systematic workflow with human verification at each stage, it enters the process at the checkpoints: a researcher reviewing an output may judge it wrong in ways they could not have pre-specified. A fully automated pipeline closes this channel. The consequence is a specific set of compounding failure modes: outputs satisfying all stated criteria while violating unstated ones; agents optimising for a simplified version of the researcher's methodology while bypassing the critical context that surrounds it; and errors entering early becoming foundational assumptions for subsequent agents, their propagation concealed by the finished document's internal coherence. Engzell and Wilmers treat these as engineering problems — better scaffolding, more adversarial critic gates. The Mitchell et al. framework suggests the inherent component is not reducible by better specification; it must enter through the verification step.
