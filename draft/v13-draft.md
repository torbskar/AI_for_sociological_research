...
The argument develops the authorship problem; introduces query authorship and shows it satisfies established authorship criteria; develops systematic use as the condition under which query authorship can be demonstrated; argues that systematic use produces an epistemic dividend by externalising tacit knowledge; describes the documentation package and its fit with open-science infrastructure; and draws the policy implications.
...
Query authorship is not as novel as it may appear: the principle — that intellectual contribution resides in the specification, and that execution by an interchangeable agent does not diminish it — is well established. What is new is the executor and its particular failure modes, not the move itself. The writing-as-thinking tradition illuminates why specification is epistemically productive: Emig (1977) argues that writing forces deliberate semantics — the explicit structuring of logical relationships that tacit thought leaves implicit — and Sommers (1981) shows that revision is recursive discovery, the process through which a prior formulation is found to be ambiguous or inconsistently held and revised accordingly. Query authorship engages exactly this process at the specification layer. Designing a search criterion, a screening rubric, or a reviewer configuration is not clerical preparation for research; it forces the researcher to articulate in explicit, structured form what would otherwise remain tacit, and through the act of writing the specification the researcher often discovers that their prior criterion was underspecified. Unsystematic use forecloses this: where the AI structures the argument and the researcher accepts the result, the deliberate semantics that force articulation of what the argument is and why the evidence supports it are bypassed. Query authorship names the point where Krippendorff's specification-authored contribution and the writing-as-thinking tradition converge in an LLM context.
...
Three markers distinguish a query that constitutes query authorship from one that does not. The first is scope logic: the query specifies what counts as in-scope and what counts as out-of-scope, and on what grounds. A literature screening instruction such as 'Retain this abstract if it addresses AI use in empirical research practice; exclude if it addresses only AI ethics in general' encodes a scope commitment. "Summarise these abstracts for me" does not. The second is evaluative grounds: the query encodes the criterion against which the output will be judged — what would make the output correct, useful, or defensible. Without this, there is nothing to verify against. The third is recognisability of failure: the query is explicit enough that the researcher can recognise when the output is wrong, not merely when it is unexpected.
...
A systematic workflow operates across five stages — literature search and screening, empirical data and analysis, drafting, review, and documentation — following the same structure at each: explicit input criteria, configured AI execution, a human verification checkpoint, and a documented artefact. The transparency artefacts produced across all stages collectively constitute the documentation package. The artefacts must be findable, structured, and interpretable to be epistemically useful: a documentation package that a reviewer cannot navigate cannot support the accountability claim it is meant to evidence. Implementation guidance for each stage and a concrete folder structure are given in the Appendix.
...
The second is what documentation *makes possible but does not certify*: whether the AI's execution of those criteria was accurate. This depends on the human verification checkpoints embedded in the workflow and on the competence of the researcher doing the verification. The framework is calibrated for researchers with deep domain expertise who can recognise when an AI output is wrong. For a researcher extending into a subfield outside their existing expertise, the framework's protections are thinnest exactly where the verification risk is highest. Human verification introduces human bias rather than eliminating it. That bias, however, has roughly knowable sources — disciplinary training, theoretical commitments, methodological tradition — making it more legible and contestable than LLM bias, whose directional effects are substantially harder to characterise and whose sources resist the scholarly norms through which human bias is typically identified and challenged. The documentation package renders human verification decisions traceable and challengeable in a way the model's inference-time processing is not; and because the two bias sources are distinct rather than correlated, human verification provides a genuinely different epistemic signal rather than compounding the same one.
...
The argument here is not that all AI use must be systematic. Researchers will continue to work exploratorily, to think while writing, and to let the process develop their thinking. What systematic AI use provides is not a constraint on how one works but evidence of how one worked.

### Open science infrastructure

The transparency artefacts that systematic use produces have the formal properties open-science infrastructure already expects: shareable, versionable, depositable, and independently evaluable. A reviewer who receives a documentation package can assess whether inclusion criteria preceded the search and whether screening decisions are reproducible from the submitted materials — the same evaluation reviewers of quantitative replication packages perform with statistical code and data. The cognitive and institutional infrastructure for this kind of review already exists. Several existing frameworks converge on this point: systematic AI use naturally satisfies the Momeni et al. (2025) checklist for computational reproducibility in social science; Törnberg (2024) shows that prompt stability analysis requires documented prompts, which systematic use provides by default; Freese and Peterson (2017) identify missing decision documentation as a persistent gap in social science open-science practice, which systematic AI use directly addresses; and Zeng et al. (2025) find that reproducibility correlates positively with accuracy across data-science tasks, suggesting systematic use may function as a quality mechanism as well as a transparency one.
...
[^3]: More elaborate practitioner implementations exist. Sant'Anna (n.d.) describes a Claude Code research workflow featuring twenty-eight configured reviewer skills, fourteen specialised agents, and AEA replication standards compliance, framed explicitly as a 'ceiling, not floor' with documented entry points for new adopters. The authorship question is not resolved by any architecture: a researcher who provided an initial prompt and reviewed a finished document faces a harder case for ICMJE criterion four than one who verified at each stage.
