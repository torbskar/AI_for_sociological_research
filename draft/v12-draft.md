...
The argument develops the authorship problem; introduces query authorship and shows it satisfies established authorship criteria; develops systematic use as the condition under which query authorship can be demonstrated, with a practical workflow across the research pipeline; argues that systematic use produces an epistemic dividend by externalising tacit knowledge; describes the documentation package and how it fits open-science infrastructure; and draws the policy implications.
...
The most direct empirical documentation of what this looks like at the level of AI-conducted research comes from Luo, Kasirzadeh, and Shah (2025), presented at the World Conference on Research Integrity in Vancouver and reported in *Science* (Jones 2026). They tested two autonomous AI scientist systems — Agent Laboratory and AI Scientist v2, both designed to execute the full research workflow without human intervention — on controlled experiments with known correct answers. They identified four failure modes: inappropriate benchmark selection, data leakage, metric misuse, and post-hoc selection bias. In practice, this included agents inventing replacement data sets when data was lost during inter-agent handoffs and running experiments multiple times while reporting only the best outcome. None of these violations were detectable from the final papers alone; identifying them required inspection of the full trace logs and code. Their recommendation follows directly: journals evaluating AI-generated research should mandate submission of trace artefacts alongside the paper. The scope of this finding matters. These are fully autonomous end-to-end systems, not researcher-guided AI use; they represent the limiting case of absent human oversight rather than a failure of systematic use with verification checkpoints. But the limiting case is analytically useful precisely because it isolates the mechanism. The failures are not a consequence of AI; they are a consequence of removing human verification from the process. The recommendation to mandate trace artefacts is the transparency argument applied to autonomous pipelines. This paper's argument begins one step earlier: verification checkpoints that prevent such failures from arising are built into systematic use as a condition of its definition.
...
A configuration file at the project root encodes the research scope and key distinctions persistently, so every session starts with the same framing. A version-control repository timestamps every change, so session logs and author-input files can be verified as contemporaneous records rather than retrospective reconstructions. Together these structural elements are not organisational convenience; they are what makes the accountability claim in the documentation package auditable in practice.
...
The second is what documentation *makes possible but does not certify*: whether the AI's execution of those criteria was accurate. This depends on the human verification checkpoints embedded in the workflow and on the competence of the researcher doing the verification. The framework is calibrated for researchers with deep domain expertise who can recognise when an AI output is wrong; for a researcher extending into a subfield outside their existing expertise, the framework's protections are thinnest exactly where the verification risk is highest. This is a genuine scope condition, not a defeater. Human verification introduces human bias rather than eliminating it; the argument for it is not that humans are unbiased but that human bias is more legible in its sources, correctable through documented checkpoints, and epistemically distinct from LLM bias — providing genuine triangulation rather than correlated confirmation.
...
The argument here is not that all AI use must be systematic, any more than the case for replication packages implies that all analysis must be automated. Researchers will continue to work exploratorily, to think while writing, and to let the process develop their thinking. What systematic AI authorship provides is not a constraint on how one works but evidence of how one worked.
...
The difficulty is that disclosure requirements, however sophisticated, do not distinguish systematic from unsystematic use. A researcher who built a documented, reproducible literature-search protocol with explicit criteria and human verification at each stage, and one who accepted unverified chatbot outputs without criteria or documentation, face the same disclosure requirement. Disclosure captures what was used and whether it was admitted; it does not capture the epistemic structure of the process. Reporting checklists share this limitation; the more than twenty instruments catalogued in recent years (Zrubka et al. 2023; Holst et al. 2025; Agha et al. 2025; Mondal et al. 2025; Ganjavi et al. 2024) address what was done and whether it was disclosed, not whether the process was epistemically sound.
