...
**Background/Problem:** The dominant institutional response to AI in academic research — disclosure requirements and binary use/non-use policies — misidentifies the epistemically relevant distinction. Whether AI was used matters less than whether that use was systematic: embedded in explicit criteria, subject to human verification, and producing documented outputs. Current policies leave this distinction invisible and treat verifiable, criterion-governed AI use identically to unverifiable, unconfigured use.

**Conceptual Framework:** I develop the distinction between systematic and unsystematic AI authorship and introduce the concept of *query authorship* — the argument that designing a search strategy, screening rubric, or reviewer configuration encodes the researcher's intellectual commitments in a form that is explicit, verifiable, and attributable. The intellectual contribution in AI-assisted research lies in the query, not in the generated text. Systematic AI authorship makes this contribution auditable; unsystematic use does not.

**Workflow:** I describe a systematic workflow for sociology and the social sciences across five pipeline stages — literature search and screening, empirical data and analysis, drafting, review, and documentation. Each stage has the same structure: explicit input criteria, a configured AI tool, a human verification checkpoint, and a documented output. The transparency artefacts produced — skill files, prompt templates, search logs, session records — constitute a replication package.

**Epistemic Properties:** Systematic AI use instantiates the epistemic function of pre-registration: it forces explicit articulation of tacit research decisions before outcomes are known. The resulting artefacts satisfy open-science norms and ICMJE authorship criteria in a way unsystematic use cannot demonstrate. Systematic use does not eliminate error; it makes errors visible and correctable.

**Policy Implications:** Journal policies should extend existing replication package norms to AI-assisted workflows — asking researchers to show what they did rather than merely whether AI was involved. This requires no new principle and no new infrastructure.
...
The second ground is mechanism. Declaration-based policy rests on the assumption that requiring a declaration produces the declared behaviour. That assumption's most prominent empirical warrant — that signing an honesty pledge at the top of a form increases honest reporting — was retracted in 2021 after data fabrication was identified in a co-authored study; two further papers by the same lead author followed in 2023 (O'Grady, 2023). The policy instrument most widely adopted has lost its flagship empirical demonstration.
...
The workflow described here is for text-heavy and empirical social science research. The argument is directed at sociology and adjacent social sciences where research questions and theoretical framings cannot be delegated to automated agents. The workflow is demonstrated using Claude (Anthropic) as the AI tool, and some implementation details use Claude-specific terminology; any AI system that supports persistent context, configurable roles, and session logging can implement the same workflow structure.
...
There is also a problem about what unsystematic AI use does to research thinking. The writing-as-thinking tradition documents that writing is epistemically valuable precisely because it forces deliberate semantics — the explicit structuring of logical relationships that tacit thought leaves implicit (Emig, 1977). Sommers (1981) frames the complement: revision is not polishing but recursive discovery — finding a structure is itself a strategy for finding meaning, and a writer who is handed a pre-given structure to polish is doing something categorically different from one who discovers structure through composition. Unsystematic AI use maps directly onto this failure: a prompt without explicit criteria, an output accepted without iteration — the AI does the structuring and the researcher accepts the result. Systematic AI use, by contrast, relocates the deliberate semantics to the specification layer: writing an inclusion criterion or a skill configuration is not clerical work but an act of explicit articulation that forces the same cognitive discipline as writing prose, while generating verifiable artefacts in the process.
...
By the time the drafting stage begins, the outline has been developed and iterated, the literature has been searched and synthesised, any empirical analysis has been conducted, and the main argument has been articulated. The drafting workflow draws on all of this as context. A skill-configured style profile encodes explicit criteria for sentence rhythm, hedging calibration, paragraph structure, and rhetorical stance. Drafts produced by a configured skill are inputs to human revision, not outputs for direct use — the researcher reads, revises, accepts, or rejects each section, exercising the judgement that makes the output theirs. The drafting stage is an opportunity to engage with writing as thinking rather than as expression: the iteration between specification and text works the same cognitive space as deliberate composition (Emig, 1977; Sommers, 1981). All prompts, configurations, and session logs are retained.
...
The third component is cross-model persona review. Configured reviewer prompts — calibrated to likely critical positions from different disciplinary and epistemological perspectives — are run in fresh sessions across at least two models from different developers. Different models have different training data, tendencies, and blind spots; a concern raised independently by two or more models is substantially more credible than the same concern raised by only one. Divergence across models signals where the argument is genuinely ambiguous. This functions as the review equivalent of a robustness check. The persona prompts and synthesis protocol — act on objections raised by two or more models; flag single-model objections for judgement; note divergence as a signal of ambiguity; disregard sycophantic positive feedback — are documented and form part of the replication package.
...
This has not been demonstrated for sociology research specifically. The claim here is more limited: systematic AI authorship is a necessary condition for the kind of quality control that makes errors detectable, and the adjacent evidence does not support the assumption of a trade-off.
